...
void joystickPanTilt() {
  ...
  if (current_time - last_input_time > INPUT_READ_INTERVAL)
  {
    ...
    int pan = map(joystick_in1, 0, 1023, 100 , 180); //motor #2 range of motion 180,100 original
    int tilt = map(joystick_in2, 0, 1023, 80, 0 ); //motor #3   range of motion 0,80 original
...
brushTip = map(brushTip,0,200,150,0);//14,280,180,0
    ...
    servo2.write(pan);
    servo3.write(tilt);
Serial.print("pan = "); Serial.println(tilt);
